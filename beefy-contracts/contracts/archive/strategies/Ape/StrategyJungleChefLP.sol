...
import "../../interfaces/common/IJungleChef.sol";
import "../Common/StratFeeManager.sol";
import "../../utils/StringUtils.sol";
import "../../utils/GasFeeThrottler.sol";

contract StrategyJungleChefLP is StratFeeManager, GasFeeThrottler {
    ...
    function deposit() public whenNotPaused {
        ...
        if (wantBal > 0) {
            IJungleChef(chef).deposit(wantBal);
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (wantBal < _amount) {
            IJungleChef(chef).withdraw( _amount - wantBal);
    ...
    function balanceOfPool() public view returns (uint256) {
        (uint256 _amount,) = IJungleChef(chef).userInfo(address(this));
        return _amount;
    }

    // returns rewards unharvested
    function rewardsAvailable() public view returns (uint256) {
        return IJungleChef(chef).pendingReward(address(this));
    }
    ...
    function retireStrat() external {
        ...
        IJungleChef(chef).emergencyWithdraw();
